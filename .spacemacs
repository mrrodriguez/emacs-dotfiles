...
This function should only modify configuration layer settings."
  (setq-default
   ...
   `(easy-kill
     ...
     environ
     apheleia
     ,@(let ((splash-path "/Users/mrodriguez/Projects/splash/stonehenge/development/emacs"))
         (when (file-exists-p splash-path)
           `((splash :location ,splash-path)))))
...
before packages are loaded."
  ...
  (with-eval-after-load 'helm
    (setq helm-boring-buffer-regexp-list
          (delete "\\*Messages\\*" helm-boring-buffer-regexp-list)))
  ...
  ;; elisp/emacs infra
  (setq find-function-C-source-directory "/Users/mrrodriguez/Projects/emacs-plus-basis/src")
  ...
  (use-package apheleia
    ...
    (setf (alist-get 'clojure-mode apheleia-mode-alist)
          'cljstyle)
    (setf (alist-get 'clojurec-mode apheleia-mode-alist)
          'cljstyle)
    (setf (alist-get 'clojurescript-mode apheleia-mode-alist)
          'cljstyle)
...
Never blocks or fails the save. Status goes to *Messages*, errors go to *Messages*."
    (when (and (derived-mode-p 'clojure-mode)
      ...
      (condition-case err
          (cider-nrepl-request:eval
           "(user/reload-nses)"
           (lambda (response)
             (nrepl-dbind-response response (out err value status)
               (when err   (message "clj-reload error: %s" err))
               (when out   (message "clj-reload: %s" (string-trim out)))
               (when value (message "clj-reload: %s" value)))))
  ...
  ;; osx - superseded by the `osx' layer config. This can be deleted if no further problems are
  ;; found.
   ...
   '(safe-local-variable-values
     '((eval let
             ...
             (when
                 (and clara-home
                      (not
                       (string-empty-p clara-home)))
               (cider-add-to-alist 'cider-jack-in-dependencies "com.github.gateless/clara-rules"
       ...
       (eval let
             ...
             (when
               ...
               (add-to-list 'cider-jack-in-dependencies "com.github.gateless/clara-rules"
                            `((("local/root" \, clara-home))))))
       (eval let
             ((clara-home
               (getenv "CLARA_HOME")))
       ...
       (eval let*
             ((project-root
              ...
              (clj-local-path
               (expand-file-name "bin/clj-local" project-root)))
             (when
                 (file-executable-p clj-local-path)
               (setq-local cider-clojure-cli-command clj-local-path)))
       (eval let
             ((clara-home
               (getenv "CLARA_HOME")))
             (when
                 (and clara-home
                      (not
                       (string-empty-p clara-home)))
               (setq-local cider-clojure-cli-global-options
                           (format "-Sdeps '{:deps {com.github.gateless/clara-rules {:local/root \"%s\"}}}'" clara-home))))
       (eval progn
             (require 'apheleia)
             (setf
              (alist-get 'cljfmt apheleia-formatters)
              '("cljfmt" "fix" "-"))
